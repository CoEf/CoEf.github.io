...
DSL 이었습니다. 
`equipment.stat_bonus`는 `"strength:1"`, `dungeon.reward_loot`는
`"아이템id:개수:확률, ..."` 식으로요. 
프로토타입으로 문자열을 Parse 해서 사용하는 방식을 채택하였지만 (이유는 문자열을 바꾸는 것만으로 데이터 타입을 정의할 수 있기 때문에)
콜론 하나만 빠뜨려도 조용히 깨지고, 에디터에서는 단순 텍스트필드로 보이기에 데이터가 많아지면 가독성이 떨어진다는 단점이 있습니다.
따라서 `ENTITY_REF`, `STRUCT`라는 두 속성 타입을 새로 만들어서 이 DSL들을 걷어냈습니다.

## 문제: 왜 문자열 DSL이 아팠나

비슷한 부류의 패턴을 정리해본 결과 총 12개의 속성이 비슷한 패턴으로 구성되어 있었습니다.
...
동작하고 있어서 여기서도 문자열 ID 참조가 일관성 있었습니다. 
대신 유저 편의를 위해 아이콘+이름이 보이는 검색 피커를 사용하는 방식을 채택했습니다.

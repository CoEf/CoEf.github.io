...
<div class="tree">
  <div class="tree-root">
    ...
    <span class="tree-desc">입력을 받고 "언제" 할지만 결정</span>
  </div>
  <ul>
    <li>
      ...
      <ul>
        ...
        <li><code>BrushComponent</code><span class="tree-desc">현재 브러시·마스크·크기·불투명도 상태</span></li>
    ...
    <li>
      ...
      <ul>
        <li><code>BrushSet</code><span class="tree-desc">브러시 1개 = PBR 텍스처 5장 묶음 (albedo/normal/ao/roughness/metallic)</span></li>
...
이 한계는 이 프로젝트를 만들고 나서 두 번째 프로젝트인 **PowerWash Simulator**를 만들면서 정면으로 부딪혔습니다. 거기서는 진행도(%)를 정확히 계산해야 했기 때문에 심 경계를 넘는 드래그가 훨씬 자주, 훨씬 눈에 띄게 문제를 일으켰고, 결국 **UV 공간에서 보간하는 방식 자체를 버리고 3D 공간에서 삼각형별로 직접 블릿하는 방식(3D 역투영)**으로 다시 설계했습니다. 그 과정은 [UV 보간의 함정과 3D 역투영](/projects/drawable-texture-powerwash/)에 정리해 두었습니다.
